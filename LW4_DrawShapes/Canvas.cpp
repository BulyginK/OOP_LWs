...
explicit CCanvas::CCanvas(HDC hdc) : m_hdc(hdc) {}

void CCanvas::DrawLine(SPoint from, SPoint to, uint32_t lineColor) const {
...
void CCanvas::FillPolygon(const std::vector<SPoint>& points, uint32_t fillColor) const {
...
void CCanvas::DrawCircle(SPoint center, double radius, uint32_t lineColor) const {
    HPEN hPen = CreatePen(PS_SOLID, 1, RGB(
        (lineColor >> 16) & 0xFF,
        (lineColor >> 8) & 0xFF,
        lineColor & 0xFF
...
void CCanvas::FillCircle(SPoint center, double radius, uint32_t fillColor) const {
    HBRUSH hBrush = CreateSolidBrush(RGB(
        (fillColor >> 16) & 0xFF,
        (fillColor >> 8) & 0xFF,
        fillColor & 0xFF
